...
public class GastosController {  @Autowired
...
public ResponseEntity<Gasto> modificarEstado(@RequestBody GastoModel gastoModel) {
 ...
 @param id El ID del cliente.
 @return La cantidad de dinero disponible en el presupuesto. */ @GetMapping("/getCantidadDinero") public ResponseEntity<Double> getCantidadDinero(@RequestParam String id) { Double cantidad = gastosUseCase.getCantidadPresupuesto(id); return ResponseEntity.ok(cantidad); }
}